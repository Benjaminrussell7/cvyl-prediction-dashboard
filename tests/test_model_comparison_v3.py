...
def test_model_comparison_v3_outputs_are_deterministic() -> None:
    rows = [
    ...
    first_comparison, first_summary = build_model_comparison_v3_outputs(pd.DataFrame(rows))
    second_comparison, second_summary = build_model_comparison_v3_outputs(
...
def test_model_comparison_v3_probabilities_are_between_zero_and_one() -> None:
    comparison, summary = build_model_comparison_v3_outputs(
        pd.DataFrame(
            [
                _game("game-1", "2026-04-10", "Avon", "Canton", 10, 5),
                _game("game-2", "2026-04-12", "Avon", "Granby", 8, 4),
...
def test_model_comparison_v3_has_no_future_leakage() -> None:
    comparison, _ = build_model_comparison_v3_outputs(
        pd.DataFrame(
            [
                _game("game-2", "2026-04-12", "Avon", "Granby", 8, 4),
    ...
    assert second["power_v3_recency_win_probability"] > 0.5


def test_model_comparison_v3_rows_and_summary_are_valid() -> None:
    games = pd.DataFrame(
        [
            _game("game-1", "2026-04-10", "Avon", "Canton", 10, 5),
            _game("game-2", "2026-04-12", "Avon", "Granby", 8, 4),
    ...
    comparison, summary = build_model_comparison_v3_outputs(games)
    ...
    assert 0 <= summary.iloc[0]["elo_accuracy"] <= 1
    assert 0 <= summary.iloc[0]["power_v2_accuracy"] <= 1
    assert 0 <= summary.iloc[0]["power_v3_recency_accuracy"] <= 1
